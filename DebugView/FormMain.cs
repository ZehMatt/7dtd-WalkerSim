		...
		Bitmap GetBitmap(Viewer.MapData mapData)
		{
			...
			using (System.Drawing.Graphics gr = System.Drawing.Graphics.FromImage(bm))
			{
				...
				if (chkInactive.Checked)
				{
					...
					{
						gr.FillEllipse(zombie.sleeper ? Brushes.Cyan : Brushes.Red, Scale(zombie.x), Scale(zombie.y), 2, 2);
					}
				}
				...
				if (chkPlayers.Checked)
				{
					...
					{
						// Zone
						gr.DrawRectangle(Pens.Red, Scale(zone.x1), Scale(zone.y1), Scale(zone.x2 - zone.x1), Scale(zone.y2 - zone.y1));
